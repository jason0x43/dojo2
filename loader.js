    ...
    function getModuleInfo(mid, referenceModule) {
        ...
        if (!(result = modules[mid])) {
            ...
            result = {
                ...
                mid: mid,
                pack: pack,
                url: compactPath((/^(?:\/|\w+:)/.test(url) ? '' : baseUrl) + url + (/\.js(?:\?[^?]*)?$/.test(url) ? '' : '.js'))
            };
        }
        return result;
    }
    ...
    function execModule(module) {
        ...
        if (!module.executed) {
            ...
            args = deps.map(function (dep) {
                if (result !== abortExec) {
                    result = ((dep === cjsRequireModule) ? createRequire(module) : ((dep === cjsExportsModule) ? module.cjs.exports : ((dep === cjsModuleModule) ? module.cjs : execModule(dep))));
                }
                return result;
            });
            if (result === abortExec) {
    ...
    var define = mix(function (deps, factory) {
        ...
        if (arguments.length === 1) {
            if (has('loader-cjs-wrapping') && typeof deps === 'function') {
                ...
                factory.toString().replace(comments, '').replace(requireCall, function () {
